...
/* ---------------------------------------------------------
   Lo que hay que revisar antes de desplegar
   Son los valores que cambian de una versión a otra: apuntan al
   sistema, no al código.
   --------------------------------------------------------- */

// Carpeta de Drive donde se crean las subcarpetas de cada estudiante.
// De acá sale el enlace "Ver mi carpeta en Drive" que se muestra al terminar
// y el que va en el correo: si este id es el del sistema viejo, los dos
// enlaces llevan al sistema viejo aunque los archivos se guarden bien.
const DRIVE_FOLDER_ID = '1P2kGjI01BXKrG-VnS7ZO3ZM1LS5gshcL';

// Planilla donde se registra cada imagen subida.
const SPREADSHEET_ID = '18KuyXMqT-eyyCaSERZPZoCYGmVVXiaefOiB3NaYm3KU';

// Dirección del formulario ya desplegado, la que termina en /exec. Va en el
// correo de aviso. Si se deja vacía se usa la del despliegue en curso, que
// sirve salvo cuando el correo se manda desde una prueba en el editor.
const URL_FORMULARIO = '';

// A quién escribirle cuando algo no cuadra. Cada estudiante tiene uno de los
// dos asignado; como acá no sabemos cuál, el correo los muestra a ambos.
const TUTORES = [
  { nombre: 'Ricardo Lagos Miranda', correo: 'ricardo.lagosm@mail.udp.cl' },
  { nombre: 'Irma Sepúlveda', correo: 'irma.sepulveda@mail.udp.cl' }
];
...
// Los meses van escritos acá y no con el patrón MMMM de formatDate:
// ese patrón usa la configuración regional del proyecto de Apps Script,
// que está en inglés, y devolvía "2 de September".
const MESES = ['enero', 'febrero', 'marzo', 'abril', 'mayo', 'junio',
  'julio', 'agosto', 'septiembre', 'octubre', 'noviembre', 'diciembre'];


function fechaEnPalabras() {
  const zona = Session.getScriptTimeZone();
  const dia = Utilities.formatDate(new Date(), zona, 'd');
  const mes = Number(Utilities.formatDate(new Date(), zona, 'M')) - 1;
  const hora = Utilities.formatDate(new Date(), zona, 'HH:mm');

  return dia + ' de ' + MESES[mes] + ', ' + hora;
}


function armarCuerpoDelCorreo(nombre, resumen, urlCarpeta) {
  const fecha = fechaEnPalabras();
  ...
  return '<p>Hola ' + nombre + ',</p>' +
    ...
    '<p style="color:#666;font-size:13px">Al guardarse, los archivos se renombran ' +
    'y son guardados en formato WebP. Los nombres de archivo que ves aquí pueden no ' +
    'corresponderse con los originales.</p>' +

    parrafoDelFormulario() +

    '<p>Frente a cualquier duda puedes comunicarte con tu tutor asignado:</p>' +
    listaDeTutores();
}

// El enlace de vuelta al formulario. Sin dirección conocida queda la frase
// sola: mejor eso que un enlace roto.
function parrafoDelFormulario() {
  const url = urlDelFormulario();

  if (!url) return '<p>Puedes volver a subir trabajos en el formulario.</p>';

  return '<p>Puedes volver a subir trabajos en el formulario ' +
    '<a href="' + url + '">aquí</a>.</p>';
}

// La dirección puesta a mano manda. getUrl() es el respaldo, y devuelve null
// cuando el script todavía no está desplegado como aplicación web.
function urlDelFormulario() {
  if (URL_FORMULARIO) return URL_FORMULARIO;

  try {
    return ScriptApp.getService().getUrl() || '';
  } catch (error) {
    Logger.log('No se pudo resolver la URL del formulario: %s', error.message);
    return '';
  }
}

function listaDeTutores() {
  const filas = TUTORES.map(function (tutor) {
    return '<li>' + tutor.nombre + ' — ' +
      '<a href="mailto:' + tutor.correo + '">' + tutor.correo + '</a></li>';
  });

  return '<ul>' + filas.join('') + '</ul>';
}
